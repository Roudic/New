...
import { FLAG_REASONS } from "@/lib/drive-thru/types";
...
export function FlagPicker({ onSelect, onCancel }: FlagPickerProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 sm:items-center">
      <div className="w-full max-w-md rounded-t-2xl bg-zinc-900 p-4 sm:rounded-2xl sm:p-6">
        ...
        <div className="space-y-2">
          {FLAG_REASONS.map((reason) => (
            <button
              ...
              className="w-full rounded-xl bg-zinc-800 px-4 py-4 text-left text-base font-medium text-white active:bg-cfa"
            >
              {reason}
            </button>
          ))}
